...
function StepIndicator({ label, status }: { label: string; status: "pending" | "active" | "done" }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={status === "active" ? "text-blue-700 font-medium" : status === "done" ? "text-green-700" : "text-muted-foreground"}>
        {label}
      </span>
    </div>
  );
      ...
      {open && (
        <div className="absolute left-0 top-7 z-50 w-80 rounded-lg border border-border bg-background shadow-lg p-4 text-sm space-y-2">
          ...
          <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
            <li>选择一个<span className="text-foreground font-medium">策略模板</span>，或手动展开各层调整参数</li>
            <li>点击右上角<span className="text-foreground font-medium">"运行选股"</span>按钮</li>
          ...
          <div className="border-t border-border pt-2 mt-2 text-xs text-muted-foreground space-y-1">
            <p><span className="font-medium text-foreground">4层漏斗：</span></p>
            <p>L1 基础过滤 → L2 趋势确认 → L3 量价择时 → L4 风控过滤</p>
            <p className="mt-1"><span className="font-medium text-foreground">提示：</span>修改任何参数会自动保存并切换为自定义模式</p>
          </div>
        </div>
      )}
    </div>
...
function SaveConfigDialog({
  ...
  const [mode, setMode] = useState<"new" | "overwrite">(existingStrategies.length > 0 ? "overwrite" : "new");
  const [name, setName] = useState("");
  const [desc, setDesc] = useState("");
  const [selectedId, setSelectedId] = useState<number | null>(existingStrategies[0]?.id ?? null);
          ...
          {existingStrategies.length > 0 && (
            <button
              ...
              className={`flex-1 px-4 py-2 transition-colors ${
                mode === "overwrite" ? "bg-primary text-primary-foreground" : "hover:bg-muted/50"
              }`}
            >
              覆盖已有策略
            </button>
...
export default function Screening() {
  ...
  const [editingStrategy, setEditingStrategy] = useState<{ id: number; name: string; description: string } | null>(null);
  ...
  const applyPresetMutation = useMutation({
    mutationFn: async (name: string) => {
      const res = await fetch(`/api/screening/apply-preset/${encodeURIComponent(name)}`, {
        method: "POST",
      });
  ...
  const [screeningStatus, setScreeningStatus] = useState<"idle" | "indicators" | "filtering" | "done" | "error">("idle");
  ...
  useEffect(() => {
    ...
    for (const msg of newMsgs) {
      if (msg.includes("[screening:start]")) setScreeningStatus("indicators");
      else if (msg.includes("[screening:indicators]")) setScreeningStatus("indicators");
      else if (msg.includes("[screening:filtering]")) setScreeningStatus("filtering");
        ...
        const match = msg.match(/(\d+)只 → L1:(\d+) → L2:(\d+) → L3:(\d+) → L4:(\d+)/);
        if (match) {
          setDoneMessage(`${match[1]}只股票 → 基础过滤:${match[2]} → 趋势确认:${match[3]} → 量价择时:${match[4]} → 风控过滤:${match[5]}`);
        } else {
          setDoneMessage("筛选完成");
        }
  ...
  const isRunning = screeningStatus === "indicators" || screeningStatus === "filtering";
  ...
  const screeningColumns: ColumnDef<ScreeningResultItem>[] = [
    ...
    { key: "pct", title: "涨跌%", align: "right", tip: "当日涨跌幅。红色为上涨，绿色为下跌。涨幅过大可能面临回调风险", render: (r) => r.pct_change != null ? <span className={(r.pct_change > 0) ? "text-red-500" : r.pct_change < 0 ? "text-green-500" : ""}>{r.pct_change > 0 ? "+" : ""}{r.pct_change.toFixed(2)}%</span> : "-" },
    { key: "amount", title: "成交额", align: "right", tip: "当日总成交金额（亿元）。越高说明资金关注度越高、流动性越好，一般 >2亿 为活跃", render: (r) => formatAmount(r.amount) },
    { key: "vol_ratio", title: "量比", align: "right", tip: "当日成交量 / 近5日平均成交量。>1.5 放量（资金涌入，积极信号），<0.8 缩量（观望），>3 异常放量需警惕", render: (r) => r.volume_ratio?.toFixed(2) ?? "-" },
    { key: "rsi", title: "RSI", align: "right", tip: "相对强弱指标(14日)。<30 超卖可能反弹（买入机会），>70 超买可能回调（风险），40-60 为中性健康区间", render: (r) => r.rsi14?.toFixed(1) ?? "-" },
    { key: "macd", title: "MACD柱", align: "right", tip: "MACD柱状图值。正值且增大=多头动能增强（好），负值且缩小=空头减弱即将反转，数值越大趋势越强", render: (r) => r.macd_hist?.toFixed(4) ?? "-" },
    { key: "ma", title: "均线", align: "center", tip: "均线多头排列状态。'多'表示MA5>MA10>MA20>MA60，是典型上涨趋势，'-'表示未形成多头排列", render: (r) => r.ma_aligned ? <span className="text-green-500 font-medium">多</span> : <span className="text-muted-foreground">-</span> },
    { key: "cross", title: "金叉", align: "center", tip: "技术指标金叉信号。M=MACD金叉（中线买入信号），K=KDJ金叉（短线买入信号），同时出现金叉共振更强", render: (r) => <div className="flex gap-1 justify-center">{r.macd_golden_cross && <span className="text-xs px-1 bg-yellow-100 text-yellow-700 rounded">M</span>}{r.kdj_golden_cross && <span className="text-xs px-1 bg-purple-100 text-purple-700 rounded">K</span>}{!r.macd_golden_cross && !r.kdj_golden_cross && "-"}</div> },
    { key: "boll", title: "BOLL位", align: "right", tip: "股价在布林带中的相对位置。0%=下轨（超卖），50%=中轨，100%=上轨（超买）。20-80%为健康区间", render: (r) => r.boll_position != null ? `${(r.boll_position * 100).toFixed(0)}%` : "-" },
    { key: "drawdown", title: "回撤%", align: "right", tip: "近20日最大回撤幅度。越小越好，<10%说明走势稳健，>20%说明近期有大幅下跌，追高风险大", render: (r) => r.max_drawdown_20d?.toFixed(1) ?? "-" },
  ...
  return (
    <div className="space-y-6">
      ...
      {isRunning && (
        <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4">
          <div className="flex items-center gap-6 text-sm">
            <StepIndicator
              label="计算指标"
              status={screeningStatus === "indicators" ? "active" : screeningStatus === "filtering" ? "done" : "pending"}
            />
      ...
      {screeningStatus === "done" && (
        <div className="text-sm text-green-600 bg-green-50 px-3 py-2 rounded-md space-y-1">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => setScreeningStatus("idle")} className="text-xs underline">关闭</button>
          </div>
          {doneMessage && <p className="text-xs text-green-700/80">{doneMessage}</p>}
        </div>
      )}
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setSaveError(null);
                      setEditingStrategy({ id: s.id, name: s.name, description: s.description });
                    }}
      ...
      <SaveConfigDialog
        ...
        error={saveError}
        existingStrategies={customStrategies?.map((s) => ({ id: s.id, name: s.name, description: s.description })) ?? []}
      />
      ...
      {config?.active_preset && (
        <p className="text-sm text-muted-foreground bg-muted/30 px-3 py-2 rounded-md">
          已使用「{config.active_preset.startsWith("custom:")
            ? customStrategies?.find((s) => s.id === Number(config.active_preset!.slice(7)))?.name ?? "自定义策略"
            : config.active_preset}」模板配置，可直接运行选股。如需微调参数，展开下方面板修改。
        </p>
      )}
      <div className="space-y-3">
        {/* Layer 1 */}
        <div className="rounded-lg border border-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">排除 ST、停牌、特定板块</span>
            </div>
            <span className="text-muted-foreground">{expandedLayer === "layer1" ? "−" : "+"}</span>
          </button>
            ...
            <div className="px-4 pb-4 space-y-2 border-t border-border pt-3">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer1: { ...config.layer1, exclude_st: !config.layer1.exclude_st } })}
                  className="rounded"
                />
                排除 ST 股票
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer1: { ...config.layer1, exclude_suspended: !config.layer1.exclude_suspended } })}
                  className="rounded"
                />
                排除停牌股票
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer1: { ...config.layer1, exclude_bse: !config.layer1.exclude_bse } })}
                  className="rounded"
                />
                排除北交所 (8xx)
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer1: { ...config.layer1, exclude_star: !config.layer1.exclude_star } })}
                  className="rounded"
                />
                排除科创板 (688xxx)
                ...
                onModeChange={(mode) =>
                  updateConfig({
                    layer1: {
                      ...
                      sector_filter: {
                        mode,
                        sector_ids: config.layer1.sector_filter?.sector_ids ?? [],
                      },
                    },
                  })
                }
        ...
        <div className="rounded-lg border border-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">均线排列、ADX、方向指标</span>
            </div>
            <span className="text-muted-foreground">{expandedLayer === "layer2" ? "−" : "+"}</span>
          </button>
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer2: { ...config.layer2, require_ma_aligned: !config.layer2.require_ma_aligned } })}
                  className="rounded"
                />
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer2: { ...config.layer2, require_ma_converge: !config.layer2.require_ma_converge } })}
                  className="rounded"
                />
                均线粘合 (MA价差收窄)
              ...
              {config.layer2.require_ma_converge && (
                <div className="flex items-center gap-2 text-sm ml-6">
                  ...
                  <NumInput
                    ...
                    onChange={(v) => updateConfig({ layer2: { ...config.layer2, ma_converge_pct: v } })}
                    step={0.5}
                    min={0.5}
                    max={10}
                  />
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer2: { ...config.layer2, require_trend_strong: !config.layer2.require_trend_strong } })}
                  className="rounded"
                />
                要求强趋势 (ADX &gt; 25)
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer2: { ...config.layer2, require_di_bullish: !config.layer2.require_di_bullish } })}
                  className="rounded"
                />
                方向看多 (+DI &gt; -DI)
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer2: { ...config.layer2, min_adx: v } })}
                  step={5}
                  min={0}
                  max={100}
                />
        ...
        <div className="rounded-lg border border-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">成交额、量比、MACD、KDJ、RSI</span>
            </div>
            <span className="text-muted-foreground">{expandedLayer === "layer3" ? "−" : "+"}</span>
          </button>
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer3: { ...config.layer3, min_amount: v * 1e8 } })}
                  step={0.5}
                  min={0}
                />
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer3: { ...config.layer3, min_volume_ratio: v } })}
                  step={0.1}
                  min={0}
                />
                <span className="text-muted-foreground">~</span>
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer3: { ...config.layer3, max_volume_ratio: v } })}
                  step={0.1}
                  min={0}
                />
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer3: { ...config.layer3, require_macd_golden_cross: !config.layer3.require_macd_golden_cross } })}
                  className="rounded"
                />
                MACD 金叉
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer3: { ...config.layer3, require_macd_positive: !config.layer3.require_macd_positive } })}
                  className="rounded"
                />
                MACD 柱为正
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  onChange={() => updateConfig({ layer3: { ...config.layer3, require_kdj_golden_cross: !config.layer3.require_kdj_golden_cross } })}
                  className="rounded"
                />
                KDJ 金叉
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer3: { ...config.layer3, rsi_min: v } })}
                  step={5}
                  min={0}
                  max={100}
                />
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer3: { ...config.layer3, rsi_max: v } })}
                  step={5}
                  min={0}
                  max={100}
                />
        ...
        <div className="rounded-lg border border-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">回撤、乖离率、WR、布林带位置</span>
            </div>
            <span className="text-muted-foreground">{expandedLayer === "layer4" ? "−" : "+"}</span>
          </button>
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, max_drawdown_limit: v } })}
                  step={1}
                  min={1}
                  max={50}
                />
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, max_consecutive_down: -Math.abs(v) } })}
                  step={1}
                  min={1}
                  max={10}
                />
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, min_bias: v } })}
                  step={1}
                  min={-30}
                  max={0}
                />
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, max_bias: v } })}
                  step={1}
                  min={0}
                  max={30}
                />
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, wr_overbought: v } })}
                  step={5}
                  min={0}
                  max={100}
                />
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <NumInput
                  ...
                  onChange={(v) => updateConfig({ layer4: { ...config.layer4, boll_upper_limit: v } })}
                  step={0.05}
                  min={0}
                  max={1}
                  className="w-24"
